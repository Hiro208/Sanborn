...
"""
v2
- Purpose: generate synthetic data with more realistic map style based on v1
- Upgrade: polygon buildings, non-square patch (7:8), aging/vignette effects, richer layout and text noise
"""
...
# Color jitter: keep hue stable, vary brightness and small noise

def random_color_jitter(base_color, mode='building'):
    """Keep hue stable and adjust brightness to avoid color drift."""
    if mode == 'bg':
...
def place_buildings_on_grid(patch_w, patch_h):
    ...
    # 1) Place a few extra-large buildings first
